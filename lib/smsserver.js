...
server.on('message', function (message, remote) {
    ...
    switch(request_type) {
        ...
        case "STATE":
            ...
            console.log('STATE response: ' + answer);
            server.send(answer, 0, answer.length, remote.port, remote.address);
            query('CALL UpdateGsmStatus(\'' + username + '\', \'' +  gsm_state + '\')',  function(results) {
                    console.log('UpdateGsmStatus result: ' + inspect(results));
            });
            break;

        case "RECORD":
            console.log('RECORD message: ' + message);
            var responseid = parseParameterValue(message_arr[0]);
            var username = parseParameterValue(message_arr[1]);
            var password = parseParameterValue(message_arr[2]);
